...
public static var current : MoodBox_js = null;
public var currentData : MoodBoxData_js;

public var bloom : MobileBloom;
public var noise : ColoredNoise;
public var fog : RenderFogPlane;

public var startMoodBox : MoodBox_js;
...
public var currentMoodBox : MoodBox_js = null;
...
function ApplyNearestMoodBoxIfDesired () {
	if (applyNearestMoodBox) { 
		...
		boxes = GetComponentsInChildren (MoodBox_js); // as MoodBox[];
		if (boxes) {
			var cameraPos : Vector3 = Camera.main.transform.position;
			var minMoodBox = boxes[0] as MoodBox_js;
			var minDistance = Mathf.Infinity;
			for (var b : Component in boxes) {
				if ( ((b as MoodBox_js).transform.position - cameraPos).sqrMagnitude < minDistance) {
					minDistance = ((b as MoodBox_js).transform.position - cameraPos).sqrMagnitude;
					minMoodBox = b as MoodBox_js;
				}
			}
			current = minMoodBox;
		}
